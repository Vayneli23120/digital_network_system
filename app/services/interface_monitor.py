...
def _count_udp_sockets() -> int:
    """统计当前进程 UDP socket 数量，用于检测泄漏"""
    try:
        pid = os.getpid()
        count = 0
        for fd_path in os.listdir(f"/proc/{pid}/fd"):
            if not fd_path.isdigit():
                continue
            try:
                link = os.readlink(f"/proc/{pid}/fd/{fd_path}")
                if "socket" in link:
                    count += 1
            except OSError:
                continue
...
class InterfaceMonitor:
    ...
    def __init__(self):
        ...
        self.retry_interval = 15         # 单台采集失败后快速重试
        self.max_concurrency = 8         # 并发采集设备数（降低防止大量 UDP socket 同时创建）
        self.snmp_timeout = 5            # SNMP walk 超时（秒），缩短以快速失败释放资源
        self.max_udp_sockets = 100       # 最大 UDP socket 数，超过时强制跳过 tick
        ...
        self._max_detached = 3            # 最多允许多少个分离线程存活
        self._stuck_since: float = 0      # 连续 stuck 开始时间戳，用于看门狗强制复位
        self._stuck_warned: bool = False  # 避免重复日志
        # 设备级熔断：连续失败 N 次后跳过该设备，避免浪费资源
        self._device_errors: Dict[int, int] = {}        # device_id → 连续失败次数
        self._max_device_errors = 5       # 连续出错上限
    ...
    def _tick_with_watchdog(self):
        ...
        增加 UDP socket 泄漏检测和熔断机制，防止 puresnmp 泄漏导致进程 FD 耗尽。
        """
        # UDP socket 泄漏检测：如果进程 FD 过多，强制跳过 tick
        try:
            fd_count = 0
            for fd_path in os.listdir(f"/proc/{os.getpid()}/fd"):
                if fd_path.isdigit():
                    fd_count += 1
            if fd_count > self.max_udp_sockets:
                logger.warning(
                    f"Too many open FDs ({fd_count}), skipping tick "
            ...
            if self._detached_count >= self._max_detached:
                ...
                if self._stuck_since == 0:
                    ...
                    logger.warning(
                        ...
                        f"and clearing inflight"
                    )
                    self._detached_count = 0
                    self._stuck_since = 0
                    self._stuck_warned = False
                    # 清除所有 inflight，让下一 tick 能重新采集
        ...
        try:
            ...
            t.start()
            t.join(timeout=self.tick_interval * 4)  # 最多等 60s
            with self._schedule_lock:
    ...
    def _run_tick_in_thread(self):
        """在独立线程中运行 _tick，使用自定义 event loop 避免 puresnmp 泄漏卡死。

        标准 asyncio.run() 在 shutdown_asyncgens() 阶段会等待 pending async
        generator 关闭。puresnmp 取消后 async generator 可能无法正常关闭，
        导致整个 event loop shutdown 卡死。这里跳过 shutdown_asyncgens，
        只关闭 event loop 本身。
        """
        ...
        try:
            ...
            # 取消所有剩余 task，但不等待 async generator 关闭
            try:
                ...
                if remaining:
                    loop.run_until_complete(
                        asyncio.wait(remaining, timeout=2)
                    )
    ...
    def _claim_due_targets(self, targets: List[Dict]) -> List[Dict]:
        ...
        with self._schedule_lock:
            ...
            for target in targets:
                device_id = target["id"]
                # 熔断检查：连续出错超过上限则跳过该设备
                if self._device_errors.get(device_id, 0) >= self._max_device_errors:
                    continue
    ...
    def _finish_target_poll(self, target: Dict, success: bool, keep_inflight: bool = False, poll_result: Optional[Dict] = None):
        ...
        if ok:
            ...
            err_count = self._device_errors.get(device_id, 0) + 1
            self._device_errors[device_id] = err_count
            if err_count >= self._max_device_errors:
                logger.warning(
                    f"Device {target['name']} ({target['ip']}) "
                    f"failed {err_count} times consecutively, skipping in future ticks"
                )
        ...
        if keep_inflight:
            status = "stuck"
        with self._schedule_lock:
            self._schedule[device_id] = time.time() + interval
            self._poll_state[target["id"]] = {
                **self._poll_state.get(target["id"], {}),
                "device_id": target["id"],
                "device_name": target.get("name", ""),
    ...
    async def _tick(self):
        ...
        logger.debug(f"Tick: {len(due)} due out of {len(targets)} targets")
        t0 = time.time()
        ...
        logger.debug(f"Tick wait: {len(done)} done, {len(pending)} pending in {time.time()-t0:.1f}s")
        cancelled_done = set()
